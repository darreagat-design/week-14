...
export function createVoiceSession(): VoiceSession {
  ...
  async function start(): Promise<void> {
    ...
    ws.onopen = async () => {
      ...
      try {
        const captureInfo = await audioCapture.start((chunk) => {
        ...
        logs.log(
          `Streaming PCM audio (${captureInfo.targetSampleRate}Hz target, ${captureInfo.inputSampleRate}Hz input, 16-bit, mono)`
        );
      } catch (err) {
        console.error(err);
        logs.log(
